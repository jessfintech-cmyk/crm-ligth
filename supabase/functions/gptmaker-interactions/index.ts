...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Buscando atendimentos do workspace:', GPTMAKER_WORKSPACE_ID);

    const response = await fetch(
      `https://api.gptmaker.ai/v2/workspace/${GPTMAKER_WORKSPACE_ID}/interactions`,
      {
        method: 'GET',
        headers: {
    ...
    console.log('Atendimentos recebidos:', data.length || 0);
    ...
    console.error('Erro em gptmaker-interactions:', error);
    return new Response(
      JSON.stringify({ 
